...
namespace Aglex
{
    public class TextPrinter : IDisposable
    {
        ...
        public void Print(Vector2d position, string text)
        {
            Print(position, Color.Transparent, text);
        }

        public void Print(Vector2d location, Color backColor, string text)
        {
            ...
            cached.Print(location);
        }

        public void Begin(GLControl glControl)
        {
            if (started && !ended) throw new AglexException("TextPrinter Begin without Flush");

            parentGlControl = glControl;

            GL.PushMatrix();

            Helpers.CreateViewportAndProjection(glControl.ClientRectangle, ProjectionTypes.Orthographic, Matrix4d.Identity);
        ...
        public void Flush()
        {
            if (!started) throw new AglexException("TextPrinter Flush without Begin");
